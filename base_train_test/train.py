...
# 构造输出路径，存储变量
out_path = Path(process['project_base']['project_path']) / pipeline['train']['output']
# 自动递归建目录文件
...
train_percentage = float(global_params['train']['train'])
batch_size = int(global_params['train']['batch_size'])
num_work = int(global_params['train']['num_work'])
epochs = int(global_params['train']['num_epochs'])
...
def train_val_data_process():
    ...
    train_transforms = transforms.Compose([transforms.Resize(size), transforms.ToTensor(), normalize])
    # 加载数据集
    data_ = ImageFolder(root=ROOT_PATH_TRAIN, transform=train_transforms)
    # 分割训练集和测试集
    train_data, val_data = Data.random_split(data_,
                                             lengths=[round(train_percentage * len(data_)), round((1 - train_percentage) * len(data_))])
...
def train_model_process(model, train_dataloader, val_dataloader, num_epochs):
    ...
    print(device)
    optimizer = torch.optim.Adam(model.parameters(), lr=lr)      # 创建优化器（梯度下降法的优化方法：SGD,Adam,A,B,C）
...
# -------------------- output --------------------
def out_put():
    best_wst_path = glob.glob('model_wts/*/*.pth')
    json.dump(best_wst_path[-1], open(out_path, mode='w', encoding='utf-8'))
...
def main():
    ...
    try:
        # 开始训练
        Train_process, Date_str_start = train_model_process(Model, Train_dataloader, Val_dataloader, num_epochs=epochs)
        # 绘制图形
